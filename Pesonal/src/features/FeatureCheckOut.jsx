...
function FeatureCheckOut() {
    ...
    if (cartItems.length <= 0) {
      dispatch(
        setDialogModal({
          ...
          texts: {
            title: 'Info',
            messageBody: 'Your Cart is Empty',
          },
        })
      );
    }
  ...
  return (
    <>
      {cartItems?.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '5vw', justifyContent: 'center' }}>
              ...
              <div className="cart-item-list">
                {cartItems.map((item, index) => (
                  <div className="flex-wrapper" key={index}>
                    ...
                    />
                    <div>{((item.quantity * item.itemPrice) / 1000).toFixed(0)}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="box-wrapper">
              <div className="flex-wrapper">
                ...
                <div style={{ textAlign: 'end' }}>
                  {(totalPrice + totalPrice * (8 / 100) + Number(tip) + 10).toFixed(1)}
                </div>
              </div>
            </div>
          </div>
